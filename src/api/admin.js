...
const { User, Alert, TopicArea, Source, Document, EmailLog, Sponsor, sequelize } = require('../models');
...
router.get('/stats', async (req, res) => {
  try {
    const [totalUsers, activeAlerts, emailsSent, documentsProcessed] = await Promise.all([
      ...
      EmailLog.count(),
      Document.count({ where: { processedAt: { [Op.not]: null } } })
    ]);
...
router.put('/api-config', async (req, res) => {
  try {
    const { cerebras_api_key } = req.body;
