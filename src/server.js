...
// User dashboard
// Dashboard route - serves different content based on auth
app.get('/dashboard', (req, res) => {
  if (req.session?.userId) {
    // Authenticated user - show user dashboard
...
// Setup route - for password setup from email verification
...
const { User, Alert, TopicArea, Source, EmailLog } = require('./models');
...
app.post('/api/test/create-temp-topic', async (req, res) => {
  try {
    ...
    const topicArea = await TopicArea.create({
      name: `[TEST] ${name}`,
            ...
            <td style="padding: 0 30px 25px 30px;">
              <div style="background-color: #f9fafb; border-left: 4px solid #3b82f6; padding: 20px; border-radius: 4px;">
                ...
                <div style="margin-bottom: 12px; font-size: 13px; color: #6b7280;">
                  <strong>${source.organization}</strong>${source.publishedAt ? ` • ${new Date(source.publishedAt).toLocaleDateString()}` : ''}
                </div>
                ...
                <a href="${source.url}" style="display: inline-block; background-color: #3b82f6; color: #ffffff; text-decoration: none; padding: 10px 20px; border-radius: 4px; font-size: 14px; font-weight: 500;">Read Full Analysis →</a>
              </div>
            </td>
          </tr>
          `).join('')}
  ...
  try {
    ...
    const testHtml = `
      <div style="padding: 20px; background: #fef3c7; border: 2px solid #f59e0b; border-radius: 8px; margin-bottom: 20px; text-align: center;">
        <strong style="color: #92400e; font-size: 16px;">⚠️ This is a Test Email</strong>
...
app.get('/api/admin/stats', async (req, res) => {
  try {
    ...
    const activeAlerts = await Alert.count({ where: { isActive: true } });
    const emailsSent = await EmailLog.count();
    const documentsProcessed = 0; // No Document model currently
...
app.get('/api/admin/users', async (req, res) => {
  try {
    ...
    const formattedUsers = users.map(user => ({
      ...
      alertsCount: user.alerts ? user.alerts.length : 0,
      interestAreas: user.alerts ? user.alerts.flatMap(a => a.keywords || []).slice(0, 5) : [],
...
app.delete('/api/admin/users/:id', async (req, res) => {
  try {
    ...
    await Alert.destroy({ where: { userId: id } });
...
app.delete('/api/admin/topics/:id', async (req, res) => {
  try {
    ...
    // Delete associated sources
    await Source.destroy({ where: { topicAreaId: id } });
    
    // Delete topic
    await TopicArea.destroy({ where: { id } });
...
app.post('/api/admin/sources', async (req, res) => {
  try {
    const { topicAreaId, name, url, type } = req.body;
...
app.post('/api/admin/sponsors', async (req, res) => {
  try {
    const { title, content, link, topicAreas } = req.body;
