    ...
    try {
      ...
      const pdfResults = searchResults
        ...
        .map(result => ({
          ...
          exaScore: result.score || 0.8, // Default score if undefined
          author: result.author,
          highlights: result.highlights
        }));

      // Step 5: Use Exa's native score as relevance (skip Cerebras to avoid errors)
      const scoredResults = pdfResults.map(result => ({
        ...result,
        relevanceScore: result.exaScore || 0.8 // Use Exa's score or default to 0.7
      }));
      ...
      const qualified = scoredResults
        .filter(r => (r.relevanceScore || 0.8) >= (minRelevanceScore || 0.5))
        .slice(0, maxResults);

      // DEBUG: Log qualified results
...
[{"index": 0, "score": 0.95}, {"index": 1, "score": 0.78}, ...]`;
  ...
  async createSources(topicAreaId, results) {
    const sources = [];
      ...
      // Validate each result is actually a PDF before creating source
